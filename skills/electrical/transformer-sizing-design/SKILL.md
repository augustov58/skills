...
4. Primary voltage (480, 480Y/277, 208…). 5. Secondary voltage (208Y/120, 480Y/277, 240/120 1φ…). 6. Phase: 1φ or 3φ — drives the √3 in every formula. 7. Connection (Δ-Y, Δ-Δ, Y-Y). Decides whether you have a **separately derived system** (Step 2). A 480Δ–208Y/120V step-down *is* an SDS; a Y-Y with a factory H0-X0 neutral link is *not*. Pass `--primary-conn` / `--secondary-conn`; the sizer infers SDS from the connection and flags Y-Y for nameplate confirmation — rerun with `--sds no` if the neutral is carried through.
...
**D2. Grounding electrode**
16b. Which electrode does the GEC land on — building steel, metal water pipe, concrete-encased (Ufer), ground rod/pipe/plate, or a ground ring? Pass `--electrode`. This is worth asking because 250.66(A)/(B)/(C) cap the GEC by electrode type (6 AWG Cu to a rod or plate, 4 AWG Cu to a Ufer, the ring conductor to a ring) and the cap is often several sizes below the table. Default `unspecified` applies no cap and shows the full Table 250.66 value. For a ring also pass `--ring-size`.
...
- **Grounding (250.30)** — script sizes the system bonding jumper and GEC, but only when it concludes the system is separately derived. Resolve the **Y-Y flag** if it fires: confirm the nameplate and rerun with `--sds no` for a factory H0-X0 link, which drops the SBJ/GEC and the riser's ground branch entirely. When it is an SDS, you still decide *where* the single N-G bond lands (transformer vs. secondary disconnect, never both). Pass `--electrode` so the 250.66(A)/(B)/(C) caps apply — a 2 AWG table GEC drops to 6 AWG on a rod or 4 AWG on a Ufer. Every cap is conditional on the run **not extending on** to another electrode type needing a larger conductor, and the script flags that condition whenever a cap fires; confirm the actual electrode arrangement before you accept the smaller conductor.
